...
def render_learning_page():
    ...
    # ---------------------------------------------------
    # ★ 추가된 부분 1: 레벨을 사람이 읽기 좋은 설명으로 변환
    # ---------------------------------------------------
    level_display = {
        "beginner": "초급자 (금융 지식이 거의 없는 입문 단계)",
        "intermediate": "중급자 (기본 개념은 알고 투자를 시작한 단계)",
        "advanced": "고급자 (심화 투자 전략과 시장 분석까지 다루는 단계)"
    }.get(level, "초급자 (금융 지식이 거의 없는 입문 단계)")
    ...
    if prompt := st.chat_input(
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(
                ...
                # -----------------------------------------
                # ★ 추가된 부분 2: system_prompt에 레벨 정보 +
                #    레벨별 설명 난이도 지시 삽입
                # -----------------------------------------
                system_prompt = f"""
당신은 대학생 금융 교육 전문가입니다.

이 학습자의 현재 수준은 다음과 같습니다: {level_display}
...
2. 학습자의 수준({level_display})에 맞춰 설명의 깊이와
   용어 난이도를 조절하세요.
   - 초급자: 전문용어를 거의 쓰지 않고, 일상적인 비유 위주로 설명
   - 중급자: 기본 용어는 그대로 쓰되 이유와 원리를 함께 설명
   - 고급자: 전문 용어(상관계수, 리스크 조정 수익률 등)를
     적극 활용하고, 실전 적용 관점에서 설명

3. 대학생이 이해하기 쉬운 표현을 사용하세요.

4. 어려운 금융 용어는 쉽게 풀어서 설명하세요.

5. 학습자의 취약 개념과 관련된 질문이라면
   더 친절하고 자세하게 설명하세요.

6. 학습 내용에 없는 내용을 임의로 만들어내지 마세요.
"""
